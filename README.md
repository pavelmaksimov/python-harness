...
```text
Core          conventional-commits · keep-a-changelog (optional) · python-tooling · python-workflow · python-libs · python-architecture · python-fsm · python-retry · python-stdlib-first-review · python-tests · python-freezegun · python-polyfactory · python-semver (libraries)
...
| `python-workflow` | Python workflow | installable | Navigate from `project/container.py` and Python modules; verify tasks through subagents; preserve reusable research; recall past changes from commit subjects with `CHANGELOG.md` details; propose uncreated commits | https://github.com/pavelmaksimov/python-harness | `harnesses/rules/python-workflow/` → `.cursor/rules/python-workflow/` |
...
| `python-architecture` | Python structure | installable | Module layout, rich Entity domain model, thin use cases, layers, adapters, domain types, `layers.toml`; sibling rule files: `python-entity.mdc`, `python-usecase.mdc`, `python-exceptions.mdc`, `python-settings.mdc`, `python-logging.mdc`, `python-di.mdc`, `python-development-rules.mdc` | https://github.com/pavelmaksimov/python-harness | `harnesses/rules/python-architecture/` → `.cursor/rules/python-architecture/`. Catalog-only `MIGRATION.md` (Service → Entity) is never copied; read it from the catalog only on an explicit migration request |
...
| `python-stdlib-first-review` | Stdlib-first review | installable | On-demand Suggest-mode review: replace hand-rolled loops and caches with itertools, functools, cachetools, collections, heapq, bisect, operator | https://docs.python.org/3/library/ | Skill: `harnesses/skills/python-stdlib-first-review/` → `.cursor/skills/python-stdlib-first-review/`; install with every core bundle. Tool from stdlib (no install); `cachetools` via `uv add cachetools` only when used |
| `python-tests` | Python tests | installable | functional tests first, modular vs e2e, stubs and mocks, HTTP mocks, no patch | https://github.com/pavelmaksimov/python-harness | `harnesses/rules/python-tests/` → `.cursor/rules/python-tests/` |
...
| `python-base-client` | HTTP adapter helper | installable | Choose httpx `AsyncApi` or `SyncApi`; AppError mapping, retries, per-call session | https://github.com/pavelmaksimov/python-harness | `harnesses/rules/python-libs/python-base-client.mdc` → `.cursor/rules/python-libs/python-base-client.mdc`; render only the selected `python-libs/ASYNC_CLIENT.md` or `SYNC_CLIENT.md` to `project/infrastructure/base/http_client.py` |
